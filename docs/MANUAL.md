...
### 13.5b MCP server (AI assistant integration)

For driving VeriFlow from Claude Code, Claude Desktop, or any other MCP
client instead of scripting against `veriflow.api` directly:
...
Exposes the same operations as `veriflow.api` as MCP tools (run
verification, list tiles/profiles, read run results, ...) plus VeriFlow's
own docs as MCP resources. No MCP client available? `veriflow context`
prints a consolidated plain-text summary you can paste into any chat
instead. Full guide, tool list, and Claude Desktop setup:
