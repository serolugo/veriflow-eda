...
Patch release — bug fixes found via end-to-end testing done after the `1.0.0` release
(including exercising the `[mcp]` extra), affecting the already-published `1.0.0` package
itself. No new features and no config/CLI/`results.json` schema changes — safe to upgrade from
`1.0.0` with no changes to `veriflow.yaml`/`project_config.yaml`. See `[Unreleased]` above for
in-progress feature work still pending its own version bump.

### Fixed

- **Broken repository links after the GitHub repo rename** (`serolugo/veriflow` →
  `serolugo/veriflow-eda`) — `setup.py`'s `project_urls` (Homepage/Repository/Issues/Changelog/
  Documentation), `README.md`'s badges and doc links, `mkdocs.yml`'s `site_url`,
  `veriflow.__homepage__` (surfaced in `veriflow doctor`'s install-help link and `cli.py`'s help
  footer), and the "Generated by VeriFlow" footer link in `veriflow wrap`'s submission README
  template all still pointed at the old repo name. The docs site link was the most urgent of the
  five: GitHub Pages has no redirect at the old URL (a real 404, not just stale content), unlike
  `github.com/.../veriflow` itself, which does redirect.
- **Yosys synthesis script: inconsistent path quoting** (`core/synth_runner.py`) —
  `read_verilog` quoted its RTL source path but `abc -liberty` didn't quote its liberty-file
  path. An unquoted path containing a space (a real, common case — a Windows user directory, or
  a PDK cache path underneath one) is silently split into multiple bogus tokens by Yosys's
  script parser instead of read as one file. Both commands now quote consistently, normalized
  through the same `Path(...).as_posix()` already used for `read_verilog`.
- **Yosys synthesis script: latch false-positive** (`core/log_parser.py`) — the latch-detection
  regex matched the substring `"latch inferred"` anywhere in the log, including inside Yosys's
  own normal, non-buggy `"No latch inferred for signal ..."` message — a design with no real
  latch could be reported as having one. Anchored the regex to line start instead.
