...
def cmd_bump_version(db: Path, tile_number: str) -> None:
    ...
    print_step("bump-version", f"New tile_id     : {new_tile_id}")
    ...
    print_step("bump-version", f"Created tiles/{new_tile_id}/")
    ...
    if old_works.exists():
        ...
        print_step("bump-version", f"Copied works/ from {old_tile_id}")
    else:
    ...
    print_step("bump-version", "Created clean runs/")
    ...
    print_step("bump-version", "Updated tile_index.csv")

    print_done("Version bumped successfully.")
    ...
    console.print(f"  [secondary]Version    [/secondary] : {parsed['id_version']:02d} -> {new_version:02d}")
    console.print(f"  [secondary]Revision   [/secondary] : {new_revision:02d} (unchanged)")
